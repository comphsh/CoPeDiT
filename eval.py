...
def inference(args):
    """
    Main inference loop: generate missing modalities for all 14 masks
    on all test samples, then compute metrics.
    """
    ...
    for sample_idx, item in enumerate(tqdm(test_files, desc="Processing samples")):
        ...
        for mask_idx, mask in enumerate(MASK_LIST):
            ...
            with autocast(enabled=True):
                ...
                _, c, _, _, _, _ = latent_avail.shape
                latent_dim = c

                latent_full = autoencoder.encode_stage_2_inputs(
                    full_img.to(device)
                ) * scale_factor
                noise_shape = (1, len(missing_idx) * latent_dim,
                               latent_avail.shape[3], latent_avail.shape[4],
                               latent_avail.shape[5])
                noise = torch.randn(noise_shape, device=device)

                full_noise = torch.zeros(
                    (1, 4 * latent_dim, latent_avail.shape[3],
                     latent_avail.shape[4], latent_avail.shape[5]),
                    device=device
                )
                avail_offset = 0
                for ai in available_idx:
                    full_noise[:, ai * latent_dim:(ai + 1) * latent_dim, :, :, :] = \
                        latent_avail[:, avail_offset, :, :, :, :]
                    avail_offset += 1
                for mi_idx, mi in enumerate(missing_idx):
                    full_noise[:, mi * latent_dim:(mi + 1) * latent_dim, :, :, :] = \
                        noise[:, mi_idx * latent_dim:(mi_idx + 1) * latent_dim, :, :, :]

                prompts = autoencoder.get_condition(x_available)

                output_latent = inferer.sample(
                    input_a=x_available,
                    input_noise=full_noise,
    ...
    # ---- Compute metrics using EVAL_SCRIPT ----
    print("\n" + "=" * 60)
    ...
    for mask_idx, mask in enumerate(MASK_LIST):
        ...
        _, _, test_files_raw = read_datalist(args.datalist_dir, args.data_root)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str, help="Directory containing test.list",
    )
    parser.add_argument(
        "--eval_script",
        default=os.environ.get(
        ...
        type=str, help="Path to the evaluation script",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Task directory for outputs (auto-generated with timestamp if not set)",
    )
