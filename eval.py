#!/usr/bin/env python
"""
CoPeDiT Inference Script for BraTS2020 Test Set — Rule-compliant.

Rules satisfied:
  Rule 0  — Global path variables (COMPARE_ROOT/DATA_ROOT/DATALIST_DIR)
  Rule 2  — Eval uses test.list, batch_size=1
  Rule 3  — Eval only generates images, no metrics
  Rule 7b — Eval output: prediction/{mask_str}/{patient}/{patient}_{mod}.nii.gz
            + 3-level tqdm (mask → patient → modality)
            + estimated/actual time print
...
def inference(args):
    ...
    # ---- Load test data (Rule 2: test.list only, never train/val) ----
    ...
    # Support both Rule 9 format and legacy format
    if "model_state_dict" in ae_state:
        autoencoder.load_state_dict(ae_state["model_state_dict"])
    else:
        autoencoder.load_state_dict(ae_state["model"])
    autoencoder.eval()
    autoencoder.requires_grad_(False)
    print(f"CoPeVAE loaded (epoch {ae_state.get('epoch', 'N/A')})")
    ...
    if "ema_state_dict" in dit_state:
        DiT.load_state_dict(dit_state["ema_state_dict"])
        print("Loaded EMA weights (Rule 9 format)")
    elif "ema" in dit_state:
        DiT.load_state_dict(dit_state["ema"])
        print("Loaded EMA weights (legacy format)")
    else:
        DiT.load_state_dict(dit_state["model"])
        print("Loaded raw model weights")
    DiT.eval()
    DiT.requires_grad_(False)
    print(f"MDiT3D loaded (epoch {dit_state.get('epoch', 'N/A')})")
    ...
    if args.noise_scheduler == "linear":
        scheduler_ddim = DDIMScheduler(
            ...
            clip_sample=False, **extra)
    else:
        extra["clip_sample"] = True
        extra.setdefault("clip_sample_min", 0)
        extra.setdefault("clip_sample_max", 1)
        scheduler_ddim = DDIMScheduler(
            num_train_timesteps=args.sample_steps,
            schedule="cosine", **extra)
    ...
    # ---- Output directory (Rule 7b) ----
    prediction_root = os.path.join(args.task_dir, "prediction")
    os.makedirs(prediction_root, exist_ok=True)

    # ---- Rule 7b: estimated total time ----
    total_tasks = len(test_files) * len(MASK_LIST)
    est_seconds_per_task = args.sample_steps * 0.15
    est_total_hours = total_tasks * est_seconds_per_task / 3600.0
    print(f"\n[INFO] Estimated total eval time: ~{est_total_hours:.1f} h "
          f"({len(MASK_LIST)} masks x {len(test_files)} patients, "
          f"DDIM steps={args.sample_steps})\n")

    # ---- Rule 7b: 3-level tqdm — mask outer loop, patient inner loop ----
    start = timeit.default_timer()

    for mask in tqdm(MASK_LIST, desc="Masks", unit="mask"):
        ...
        for item in tqdm(test_files, desc=f"  Patients ({mask_str})", leave=False):
            subject_id = item["subject_id"]

            try:
                data = eval_transform(item)
            except Exception as e:
                tqdm.write(f"Warning: Failed to load {subject_id}: {e}")
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ae_ckpt", required=True, type=str,
                        help="Path to trained CoPeVAE checkpoint")
    parser.add_argument("--dit_ckpt", required=True, type=str,
    ...
    parser.add_argument("--task_dir", default=None, type=str,
